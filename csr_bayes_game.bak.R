...
# ## BW PLOT
# png('csr_demand_q_z_ggplot_box_facet.png', height=6,width=8, units='in', res=300)
# p <- ggplot(aes(x=q,y=counts), data=df.comb) + 
#   # stat_smooth(se=T, span=0.9999) +
#   geom_boxplot(aes(group=q)) +
#   # geom_line() + 
#   # geom_point() +
...
# GET RIBBON CONF INTERVALS
df.ci <- data.frame(z=NA, q=NA, Platform=NA, L95=NA, mu=NA, U95=NA)
counter <- 1
for (i in levels(df.counts$z)) {
  for (j in unique(df.counts$q)) {
    for (k in levels(df.counts$Platform)) {
      ...
      df.ci[counter,c('L95','mu','U95')] <- quantile(na.omit(x), probs = c(.025, .5, .975))
      counter <- counter + 1
    }
  }
}
...
p <- ggplot(aes(x=as.numeric(q),y=mu), data=na.omit(df.ci)) +
  ...
  geom_ribbon(aes(group=z,ymin=L95,ymax=U95), alpha=.35) +
  facet_wrap( ~ Platform) +  theme_bw() +
  xlab('Proportion of Hedonic Buyers (q)') + 
  ylab('Quantity Demanded')
p
ggsave('demand_by_q_z_facets_ribbon_ggplot_3.png', height=4,width=7.5,units='in',dpi=300)
