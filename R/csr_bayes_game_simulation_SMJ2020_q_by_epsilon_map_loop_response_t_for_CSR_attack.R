...
##-----------------------------------------
n   <- 11  ## granularity (length of contingency vectors to simulate)
...
#=====================================================================
#----------------- 3. CSR RESPONSE LOOP RESPONSE TIMING SIMULATION ----------------------
#---------------------------------------------------------------------
...
t1.changes <- sapply(2^(1:12),function(t)ifelse(t < x$Tau, t, x$Tau)) # x$Tau  #  c(30, 90, 270) # c(20, 80, 320) 
##-----FIXED PARAMS-----------------------------
...
for (m in 1:length(t1.changes)) {
  ...
  idstr <- sprintf('_uber_l_list_facet_plot_CSR_CSR_T_%s_w_%s_J1_%s_J2_%s_gamma1_%s_discount1_%s_gn_%s_t1_%s',
                   x$Tau,x$omega,x$J1.0,x$J2.0,x$gamma1,x$discount1,n,t1.changes[m]) 
