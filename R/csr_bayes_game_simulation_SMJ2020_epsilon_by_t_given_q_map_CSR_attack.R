...
##-----------------------------------------
n   <- 21  ## granularity (length of contingency vectors to simulate)
t2r <- 100  ## response timing (if the response action is CSR or PRICE)

#=====================================================================
#----------------- 1. CSR RESPONSE TIMING by NETWORK EFFECTS  ------------
#---------------------------------------------------------------------

##----RESPONSE ACTION (platform 2) ------------
gamma1s    <- 0.2 # CSR action (0 < gamma1 < (1-c1)) #where c is marginal cost
...
qs <- 0.75 #LOW ## round(seq(1,0, length.out = n), 2)  # seq(0,1,.1) ##c(0,.02,.05,.1,.2,.3,.4) # c(0,.2,.4,.6,.8,1)  #seq(0,1,.1)
epsilons <- round(seq(0,2, length.out = n), 2)  #
rs <- sapply(2^(0:12), function(t)ifelse(t < x$Tau, t, x$Tau))  #response timing
##----init data objects-------------------------
l.list <- list()  
z <- matrix(NA, nrow = length(epsilons), ncol=length(rs), dimnames = list(rownames=epsilons,colnames=rs))
      ...
      for (m in 1:length(t1.changes)) {
        for (a in 1:length(gamma1s)) {
          for (b in 1:length(discount1s)) {
            for (h in 1:length(qs)) {
              t1.response <- ifelse(rs[i]>=x$Tau,'NO',
              ...
              x$t1.change <- rs[i] ##t1.changes[m] ## RESPONSE TIMING
...
filled.contour(xx, yy, tz,  
               ...
               xlab=expression('2^t Response Pd '),
...
idstr <- sprintf('_uber_l_list_facet_plot_CSR_CSRResponse_variable_T_%s_w_%s_J1_%s_J2_%s_gamma1_%s_discount1_%s_gn_%s_t1_%s',
...
dbs <-  0.1 # sellers churn # c(0.05, 0.1, 0.2) #c(.05,.5)
##----CONTINGENCIES TO TEST---------------------
qs <- round(seq(1,0, length.out = n), 2)  # seq(0,1,.1) ##c(0,.02,.05,.1,.2,.3,.4) # c(0,.2,.4,.6,.8,1)  #seq(0,1,.1)
