...
G <- list(
  ...
  psi.s = c(.2, .2),         ## seller CSR incremental marginal cost  ##price##
  c.p = c(.5, .5),               ## platform marginal cost   
  psi.p = c(0.2, 0.2),         ## platform CSR incremental marginal
  r = c(.1, .1),               ## platform transaction fee rate
  E.s = c(1, 1),           ## seller fixed cost
  R.s = c(1, 1),           ## seller CSR fixed cost
  E.p = c(1, 1),           ## platform fixed cost
  R.p = c(1, 1),           ## platform CSR fixed cost
  Y = 1000,                    ## Budget
  ## initialize
  init = function(...) {
    ...
    G <- init.buyers(G)
    G <- init.platform(G)
    G$p.CSR <- (G$epsilon+1) * G$c.s[1] * (1 + G$psi.s[1])
    G$p.NO <- (G$epsilon+1) * G$c.s[1]
    G$s.CSR.U <- .share(G$p.CSR,G$p.CSR,G$u[1],G$u[2],1,1,G$J[1],G$J[2],G$omega,G$epsilon,'U',1)
    G$s.CSR.H <- .share(G$p.CSR,G$p.CSR,G$u[1],G$u[2],1,1,G$J[1],G$J[2],G$omega,G$epsilon,'H',1)
    G$s.NO.U  <- .share(G$p.CSR,G$p.CSR,G$u[1],G$u[2],0,1,G$J[1],G$J[2],G$omega,G$epsilon,'U',1)
    G$s.NO.H  <- .share(G$p.CSR,G$p.CSR,G$u[1],G$u[2],0,1,G$J[1],G$J[2],G$omega,G$epsilon,'H',1)
    ##
    G$piCSR.H <- G$s.CSR.H * ((G$r[1] * (G$p.CSR - G$c.p - G$psi.p))/G$p.CSR) * G$N * G$Y - (G$E.p + G$R.p)
    G$piCSR.U <- G$s.CSR.U * ((G$r[1] * (G$p.CSR - G$c.p - G$psi.p))/G$p.CSR) * G$N * G$Y - (G$E.p + G$R.p)
    G$piNO.H <- - G$E.p
    G$piNO.U <- ((G$r * (G$p.NO - G$c.p - G$psi.p))/G$p.NO) * G$N * G$Y - G$E.p
    ## check BPE ? 
      ## TODO
    G$init = NULL
    return(G)
...
#---------------------------------------
pl <- list()
Ns <-   100000 # c(10,100, 1000, 10000,100000,40000000)
