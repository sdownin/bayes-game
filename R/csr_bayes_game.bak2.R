...
s0[i] <- ((p2*v1/p1*v2)*pow(J1, rho)) / ( (p2*v1/p1*v2)*pow(J1, rho) + pow(J2, rho) )
G[i] ~ ifelse(sig2 > sig1, L-dbinom(s[i],L), dbinom(s[i],L) )
}
q ~ dbeta(h1t,h2t)
}
"

modelstring1 <- function(sig1,sig2)
{
  paste0("model{
         for (i in 1:n) {
         z[i] ~ dbern(q)
...
sig1 <- 0
sig2 <- 1
modelstring <- modelstring1(sig1,sig2)
Y <- 2000
