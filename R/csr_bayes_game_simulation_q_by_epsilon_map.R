...
epsilons <- round(seq(.5,2.1, length.out = n), 2)  #
dbs <-  0.1 # c(0.05, 0.1, 0.2) #c(.05,.5)
phis <- .1   # CSR cost-base price increase
t1.changes <- 2499 # x$Tau  #  c(30, 90, 270) # c(20, 80, 320) 

z <- matrix(NA, nrow = n, ncol=n, dimnames = list(rownames=epsilons,colnames=qs))
...
file.name <- sprintf('equilib_demand_share_phi%.2f_respPd%s_Tau%s_db%s_J1%s_J2%s_omega%s.png', phis[1],t1.changes[1],x$Tau,dbs[1],x$J1.0,x$J2.0,x$omega)
...
filled.contour(xx, yy, tz,  
               ...
               xlab=expression('Hedonic Proportion '~(q)),
